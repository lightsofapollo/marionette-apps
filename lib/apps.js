...
var Mgmt = require(__dirname + '/mgmt'),
    fs = require('fs');
...
function Apps(client) {
  this._client = client;
  this.mgmt = new Mgmt(this, client);
}
module.exports = Apps;
 ...
 */
Apps.setup = function(client, options, cb) {
  ...
  apps._client.setContext('chrome', function(err) {
    if (err) {
      throw err;
    }

    apps._prepareClient(function(err) {
      if (err) {
        throw err;
      }
...
Apps.prototype = {
   ...
   * @private
   */
  _prepareClient: function(cb) {
    // A collection of scripts we want to load into b2g.
    var paths = [
      __dirname + '/scripts/objectcache.js',
      __dirname + '/scripts/utils.js'
    ];

    // Load the scripts and call the callback once they finish.
    var pending = paths.length;
    paths.forEach((function(path) {
      var script = fs.readFileSync(path, 'utf8');
      this._client.importScript(script, function() {
        pending -= 1;
        if (pending === 0) {
          cb && cb();
        }
